...
const ProfileDetails = ({ user }) => {
  ...
  return (
    <div className="flex flex-col relative w-6/12 ml-auto h-max">
      {/* details */}
      <div
        className={`w-8/12 flex flex-col h-max p-4 pl-7 left-0 ${style.blueBlur} ${style.rounded}`}
      >
        {/* FullName and userName */}
        <p className="w-8/12 text-blue_light_3 text-2xl text-center">
          {user.fullName}
          <span className={`block text-lg text-blue_light_4`}>{user.username}</span>
        </p>
        ...
        <Link to={"/games"} className="w-8/12 text-center text-lg text-orange_3 mt-5">
          view more
        </Link>
      </div>
      {/* image */}
