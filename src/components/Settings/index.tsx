  ...
  return (
    <div className="animate-success-check w-full text-center">
      <img
        src={checkMark}
        alt="checkMark"
        className="mx-auto mb-5 max-w-full h-16"
      />
...
*/
  ...
  useEffect(() => {
    ...
    const fetch2FAStatus = async () => {
      try {
        const response = await fetch(config.domain + "/2fa/status", {
          method: "GET",
          headers: Header,
        });
        ...
        if (response.ok) {
          ...
          setTest(is_twofactor);
          if (is_twofactor) setChecked(true);
        } else {
...
*/
  const disable2FA = async () => {
    try {
      const response = await fetch(config.domain + "/2fa/disable", {
        method: "GET",
        headers: Header,
      });
  ...
  const fetchQrCode = async () => {
    try {
      if (setTest != false) {
        ...
        if (!response.ok) {
          throw new Error(
            `Failed to fetch QR code. Status: ${response.status}`
          );
        }
